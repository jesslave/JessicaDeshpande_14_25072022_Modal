...
function shouldIgnore(context, ignore, only, dirname) {
  if (ignore && matchesPatterns(context, ignore, dirname)) {
    ...
    const message = "No config is applied to \"".concat((_context$filename = context.filename) != null ? _context$filename : "(unknown)", "\" because it matches one of `ignore: ").concat(JSON.stringify(ignore, ignoreListReplacer), "` from \"").concat(dirname, "\"");
    debug(message);
  ...
  if (only && !matchesPatterns(context, only, dirname)) {
    ...
    const message = "No config is applied to \"".concat((_context$filename2 = context.filename) != null ? _context$filename2 : "(unknown)", "\" because it fails to match one of `only: ").concat(JSON.stringify(only, ignoreListReplacer), "` from \"").concat(dirname, "\"");
    debug(message);
