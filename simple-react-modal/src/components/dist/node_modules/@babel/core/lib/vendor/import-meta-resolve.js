...
const subset = function subset(sub, dom) {
...
codes.ERR_INVALID_MODULE_SPECIFIER = createError('ERR_INVALID_MODULE_SPECIFIER', function (request, reason) {
  let base = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : undefined;
...
codes.ERR_INVALID_PACKAGE_TARGET = createError('ERR_INVALID_PACKAGE_TARGET', function (pkgPath, key, target) {
  let isImport = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : false;
  let base = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : undefined;
...
function legacyMainResolve(packageJsonUrl, packageConfig, base) {
  ...
  if (packageConfig.main !== undefined) {
    ...
    const tries = ["./".concat(packageConfig.main, ".js"), "./".concat(packageConfig.main, ".json"), "./".concat(packageConfig.main, ".node"), "./".concat(packageConfig.main, "/index.js"), "./".concat(packageConfig.main, "/index.json"), "./".concat(packageConfig.main, "/index.node")];
    let i = -1;
...
function throwInvalidSubpath(subpath, packageJsonUrl, internal, base) {
  const reason = "request is not a valid subpath for the \"".concat(internal ? 'imports' : 'exports', "\" resolution of ").concat((0, _url().fileURLToPath)(packageJsonUrl));
  throw new ERR_INVALID_MODULE_SPECIFIER(subpath, reason, base && (0, _url().fileURLToPath)(base));
}

function throwInvalidPackageTarget(subpath, target, packageJsonUrl, internal, base) {
  target = typeof target === 'object' && target !== null ? JSON.stringify(target, null, '') : "".concat(target);
