...
function traceMappings(tree) {
  const gen = new _genMapping.GenMapping({
    file: tree.map.file
  });
  const {
    sources: rootSources,
  ...
  const rootMappings = (0, _traceMapping.decodedMappings)(map);
  ...
  for (let i = 0; i < rootMappings.length; i++) {
    ...
    for (let j = 0; j < segments.length; j++) {
      ...
      (0, _genMapping.addSegment)(gen, i, genCol, source, line, column, name);
      if (content != null) (0, _genMapping.setSourceContent)(gen, source, content);
    }
  }
...
function originalPositionFor(source, line, column, name) {
  ...
  const segment = (0, _traceMapping.traceSegment)(source.map, line, column); // If we couldn't find a segment, then this doesn't exist in the sourcemap.
...
function buildSourceMapTree(input, loader) {
  const maps = asArray(input).map(m => new _traceMapping.TraceMap(m, ''));
  const map = maps.pop();
...
function build(map, loader, importer, importerDepth) {
  ...
  const children = resolvedSources.map((sourceFile, i) => {
    ...
    if (sourceMap) return build(new _traceMapping.TraceMap(sourceMap, source), loader, source, depth); // Else, it's an an unmodified source file.
...
class SourceMap {
  constructor(map, options) {
    const out = options.decodedMappings ? (0, _genMapping.decodedMap)(map) : (0, _genMapping.encodedMap)(map);
